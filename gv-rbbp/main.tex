...
\caption{Maximum flow for a network with $n=5$, $m=3$ and $b=[2,2,1]$.
...
By the max-flow-min-cut theorem, the max-flow is at most $n$.
...
by the max-flow integrality theorem, there is an integral max-flow $f$ of value $n$.
...
\State $E' = \{(s, u_i, 1): \forall i \in I\}
    \cup \{(u_i, v_j, 1): \forall (i, j) \in E\}
    \cup \{(v_j, t, b_j): \forall j \in J\}$
\State Get an integral max-flow $f$ for $G' = (V', E')$ using the Ford-Fulkerson algorithm.
...
and all items in $\widehat{B}^w_{p,q,r}$ have width $q\epsLarge^2/4$ and
height at most $h^w_{p,q,r}$.
Therefore, $\widehat{B}^w_{p,q,r} \preceq B^w_{p,q,r}$.
Similarly, $\widehat{B}^h_{p,q,r} \preceq B^h_{p,q,r}$.
\end{proof}
