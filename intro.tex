...
We will first introduce the classical bin packing problem and the classical knapsack problem,
which are important problems in computer science and operations research.
We will then look at well-known generalizations of these problems.
This will set the context for us to describe the contributions of this thesis.
...
\section{Classical Knapsack Problem}
...
\section{Geometric Bin Packing Problem}
...
%
There are two commonly-studied versions of the above problem.
...
2D GBP is also used for placing advertisements on pages.
Here rotating items is forbidden.
...
3D GBP has applications in logistics,
where boxes need to be packed into cuboidal shipping containers.
Here usually the boxes can be rotated in any way, but there are exceptions:
...
In the non-rotational version of 2D GBP, we can assume \wLoG{} that
the bin is a square of side length 1.
...
Chleb{\'\i}k and Chleb{\'\i}kov\'a~\cite{chlebik2009hardness} prove that it is NP-hard to solve
...
For the version of 3BP where items can be rotated about all axes,
...
\section{Geometric Knapsack Problem}
...
\section{Vector Bin Packing Problem}
...
Khan's PhD thesis~\cite{khan-thesis} shows how to improve the hardness to $d^{1-\eps}$.
...
\section{Vector Knapsack Problem}
